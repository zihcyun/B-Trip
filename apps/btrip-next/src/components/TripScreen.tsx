...
const TripScreen: React.FC = () => {
  const activeTrip = MOCK_TRIPS.find((trip) => trip.isActive);
  ...
  return (
    <div className="h-full bg-white overflow-y-auto no-scrollbar pb-20">
      ...
      <div className="px-4 mb-10">
        ...
        <button className="mt-6 px-5 py-2 border border-black rounded-full text-xs font-bold">
          編輯行程
        </button>
      </div>
      ...
      <div className="px-4">
        ...
        <div className="space-y-3">
          {MOCK_TRIPS.map((trip) => (
            <div
              key={trip.id}
              className={`flex border rounded-xl overflow-hidden shadow-sm h-16 ${
                trip.isActive ? "bg-gray-50 opacity-60" : ""
              }`}
            >
